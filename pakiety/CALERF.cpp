...
long double  calerfpack::calerf_LD(const long double arg, const int jint)
{
//-----------------------------------------------------------------
//
//  This packet evaluates erf(x), erfc(x), and exp(x*x)*erfc(x)
//  for a real argument x.  It contains four long double functions:
//  erf_LD(), erfc_LD(), and erex_LD(), and calerf_LD().
//
...
//         y = erex_LD(x).
//
//  The function calerf_LD() is intended for internal packet use only,
//  all computations within the packet being concentrated in this
//  routine. The functions invoke calerf_LD() with the
//  statement
//
//         calerf_LD(arg,jint)
//
...
//
//      Function                     Parameters for calerf_LD()
//       call       l       arg            jint
//
//     erf_LD(arg)     ANY REAL ARGUMENT      0
//     erfc_LD(arg)    fabs(arg)  < XBIG      1
//     erex_LD(arg)    XNEG < arg < XMAX      2
//
...
//   XNEG   = the largest negative argument acceptable to erex_LD();
...
//   XBIG   = largest argument acceptable to erfc_LD();  solution to
//            the equation:  W(x) * (1-0.5/x**2) = XMIN,  where
//            W(x) = exp(-x*x)/[x*sqrt(pi)].
//   XHUGE  = argument above which  1.0 - 1/(2*x*x) = 1.0  to
//            machine precision.  A conservative value is
//            1/[2*sqrt(XSMALL)]
//   XMAX   = largest acceptable argument to erex_LD(); the minimum
...
// Używać odpowiedniego kompilatora, który będzie w stanie uzyskać taką precyzję
//-----------------------------------------------------------------
...
long double  calerfpack::erf_LD(const long double x)
{
//-------------------------------------------------------------------
//  This function computes approximate values for erf(x).
//  (see comments heading calerf_LD()).
//
//  Based on the netlib package by W. J. Cody, January 8, 1985
//-------------------------------------------------------------------
return  calerfpack::calerf_LD(x,0);
}
...
long double  calerfpack::erfc_LD(const long double x)
{
//-------------------------------------------------------------------
...
//  Based on the netlib package by W. J. Cody, January 8, 1985
//-------------------------------------------------------------------
return  calerfpack::calerf_LD(x,1);
}
...
long double  calerfpack::erex_LD(const long double x)
{
//-----------------------------------------------------------------
...
//-----------------------------------------------------------------
return  calerfpack::calerf_LD(x,2);
}
